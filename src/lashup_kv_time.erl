...
handle_mc_event(Origin, Event = #{type := tick, system_time_ms := OriginalTime}, State) ->
  lager:debug("Received Tick from ~p: ~p", [Origin, Event]),
...
handle_mc_event(Origin,
  ...
  lager:debug("Received Tick Reply from ~p: ~p", [Origin, Event]),
  Now = erlang:system_time(milli_seconds),
  Entry =
  #rtt_entry{
